...
<div class="flex flex-col max-w-lg p-6 mx-auto rounded-md sm:p-10">
	<div class="mb-8 text-center">
		<h1 class="my-3 text-4xl font-bold">Edit Task</h1>
	</div>
	<form method="post" action="/tasks" class="space-y-5 ng-untouched ng-pristine ng-valid">
		@csrf
        @method('PUT')
		<div class="space-y-4">
            ...
            <div>
                ...
                <div class="inline-block relative w-full">
                    ...
                    ">
                        <option
                        @if (old('category') == 'Urgent and Important' || $task->category == 'Urgent and Important')
                            selected
                        @endif
                        ...
                        <option
                        @if (old('category') == 'Not Urgent but Important' || $task->category == 'Not Urgent but Important')
                            selected
                        @endif
                        ...
                        <option
                        @if (old('category') == 'Urgent but Not Important' || $task->category == 'Urgent but Not Important')
                            selected
                        @endif
                        ...
                        <option
                        @if (old('category') == 'Not Urgent and Not Important' || $task->category == 'Not Urgent and Not Important')
                            selected
                        @endif
            ...
            <div>
                ...
                <div class="inline-block relative w-full">
                    ...
                    ">
                        <option
                        @if (old('priority') == 'High' || $task->priority == 'High')
                            selected
                        @endif
                        value="High">
                            High
                        ...
                        <option
                        @if (old('priority') == 'Medium' || $task->priority == 'Medium')
                            selected
                        @endif
                        value="Medium">
                            Medium
                        ...
                        <option
                        @if (old('priority') == 'Low' || $task->priority == 'Low')
                            selected
                        @endif
                        value="Low">
                            Low
            ...
            <div>
                ...
                <div class="tags-inserted flex flex-wrap gap-1">
                    @foreach ($task->tags as $tag)
                        <span class="px-3 py-1 rounded-full border border-primary mr-2 hover:bg-error hover:text-white cursor-pointer">{{ $tag->name }}</span>

                        <script>
                             // insert into tags-result
                            document.querySelector('#tags-result').value += '{{ $tag->name }} ';
                            console.log(document.querySelector('#tags-result').value);

                            // remove tag
                            document.querySelector('.tags-inserted').addEventListener('click', function(e) {
                                if (e.target.classList.contains('hover:bg-error')) {
                                    e.target.remove();
                                    document.querySelector('#tags-result').value = document.querySelector('#tags-result').value.replace(e.target.textContent, '');
                                    document.querySelector('#tags-result').value = document.querySelector('#tags-result').value.trim();
                                }
                            })
                        </script>
                    @endforeach
        ...
        <button type="submit" class="w-full px-8 py-3 font-semibold rounded-md bg-primary text-primary-content mt-5">Save</button>
	</form>
        <script>
